...
struct RegFile {
	...
	RegFile() { memset(regs, 0, sizeof(regs)); }

	RegFile(const RegFile &other) { memcpy(regs, other.regs, sizeof(regs)); }
	...
	Taint<int32_t> &operator[](const uint32_t idx) { return regs[idx]; }

	void show() {
		for (int i = 0; i < NUM_REGS; ++i) {
			printf("\e[38;5;%um%s\e[39m = 0x%08x %s\n", 100 + i, regnames[i], *reinterpret_cast<uint32_t *>(&regs[i]),
			       regs[i].getTaintId() > 0 ? "*" : "");
		}
	}
};
