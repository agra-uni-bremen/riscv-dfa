...
void DebugCoreRunner::send_packet(int conn, const std::string &msg) {
	...
	unsigned nbytes = ::send(conn, iobuf, frame.size(), 0);
	assert(nbytes == frame.size());
}

void send_raw(int conn, const std::string &msg) {
	unsigned nbytes = ::send(conn, msg.c_str(), msg.size(), 0);
...
void DebugCoreRunner::handle_gdb_loop(int conn) {
	while (true) {
		...
		if (msg.size() == 0) {
			...
			std::string ans;
			// NOTE: reading from a memory mapped device is currently not supported (can implement a *debug-read* method
			// on the bus or similar)
