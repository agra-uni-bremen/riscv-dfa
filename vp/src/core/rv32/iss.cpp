...
Opcode::Mapping ISS::exec_step() {
	...
	DEBUG(printf("%s ", Opcode::mappingStr[op]); switch (Opcode::getType(op)) {
		case Opcode::Type::R:
			printf("\e[38;5;%um%s\e[39m, \e[38;5;%um%s\e[39m, \e[38;5;%um%s\e[39m", 100 + instr.rd(),
			       regnames[instr.rd()], 100 + instr.rs1(), regnames[instr.rs1()], 100 + instr.rs2(),
			       regnames[instr.rs2()]);
			break;
		case Opcode::Type::I:
			printf("\e[38;5;%um%s\e[39m, \e[38;5;%um%s\e[39m, 0x%x", 100 + instr.rd(), regnames[instr.rd()],
			       100 + instr.rs1(), regnames[instr.rs1()], (uint32_t)instr.I_imm());
			break;
		case Opcode::Type::S:
			printf("\e[38;5;%um%s\e[39m, \e[38;5;%um%s\e[39m, 0x%x", 100 + instr.rs1(), regnames[instr.rs1()],
			       100 + instr.rs2(), regnames[instr.rs2()], (uint32_t)instr.S_imm());
			break;
		case Opcode::Type::B:
			printf("\e[38;5;%um%s\e[39m, \e[38;5;%um%s\e[39m, 0x%x", 100 + instr.rs1(), regnames[instr.rs1()],
			       100 + instr.rs2(), regnames[instr.rs2()], (uint32_t)instr.B_imm());
			break;
		case Opcode::Type::U:
		...
		switch (op) {
			...
			case Opcode::SB: {
				...
				DEBUG(std::cout << "\t = *(" << regs[instr.rs1()] << " + " << instr.S_imm() << ") <- "
				                << *reinterpret_cast<uint32_t *>(&regs[instr.rs2()]) << std::endl);
				break;
			}
			...
			case Opcode::BLTU:
				...
				if (regs[instr.rs1()].as<uint32_t>() < regs[instr.rs2()].as<uint32_t>()) pc = last_pc + instr.B_imm();
				break;
			...
			case Opcode::ECALL: {
				// NOTE: cast to unsigned value to avoid sign extension, since execute_syscall expects a native 64 bit
				// value
				int ans = sys->execute_syscall(regs[RegFile::a7].as<uint32_t>(), regs[RegFile::a0].as<uint32_t>(),
				                               regs[RegFile::a1].as<uint32_t>(), regs[RegFile::a2].as<uint32_t>(),
				                               regs[RegFile::a3].as<uint32_t>());
				regs[RegFile::a0] = ans;
			} break;
			...
			case Opcode::MULHU: {
				DEBUG(std::cout << "\t" << regs[instr.rs1()] << " * " << regs[instr.rs2()]);
				Taint<uint64_t> ans =
				    regs[instr.rs1()].as<uint32_t>().as<uint64_t>() * regs[instr.rs2()].as<uint32_t>().as<uint64_t>();
				regs[instr.rd()] = (ans & 0xFFFFFFFF00000000) >> 32;
				DEBUG(std::cout << " = " << regs[instr.rd()] << std::endl);
				break;
			}
			...
			case Opcode::LR_W: {
				// TODO: in multi-threaded system (or even if other components can access the memory independently, e.g.
				// through DMA) need to mark this addr as reserved
	...
	// NOTE: writes to zero register are supposedly allowed but must be ignored (reset it after every instruction,
	// instead of checking *rd != zero* before every register write)
